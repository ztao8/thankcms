...
// ajax获取对应的城市
function ajaxCityList(parentid,city_id){
	if(parentid != '0' && parentid != ''){
		$.getJSON(
			"/city/ajaxCityList.html?parentid="+parentid,
			function(data){
				var html = '';
				$.each(data,function(n,val) {  
					...
					html += '<option '+str+' value="'+val['id']+'">'+val['name']+'</option>';
				});
				$('#city_id').append(html);
			}
		);
...
//ajax获取对应的区域
function ajaxAreaList(parentid,city_id,area_id){
	if(city_id != '0' && city_id != ''){
		$.getJSON(
			"/area/ajaxAreaList.html?parentid="+parentid+'&city_id='+city_id,
			function(data){
				var html = '';
				$.each(data,function(n,val) {  
					var str = '';
